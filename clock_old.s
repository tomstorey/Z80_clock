...
BTN_ENT_BIT       .EQU  2
BTN_ESC_BIT       .EQU  3
BTN_UP_MASK       .EQU  1 << BTN UP_BIT
BTN_DN_MASK       .EQU  1 << BTN_DN_BIT
...
;     the OTIR instruction can be used to do a two byte transfer from
;     ROM to the display driver data register.
;
...
DO_ROW_NEXT:
      ...
      LD    A, (HL)                 ; HL now points to DP byte
      OUT   (C), A                  ; Shift it out

;---- Writing the control byte causes the shifted data to be latched
;     and the next row of the display to be selected per control byte.
      LD    C, DISP_CTRL            ; Load display control byte and
      LD    A, (DISP_CONTROL)       ; write to control register.
      OUT   (C), A

;---- Finish up by incrementing the row counter in the control byte.
